...
- GUI/computer use never runs in the main loop - see Computer Use / GUI Automation for executor routing (a separate root-managed Orca Codex Task/Dispatch for approval-seeded desktop and device work, haiku/sonnet subagents otherwise)

Claude Code (Fable-led task):
- The Fable main loop keeps architecture, decomposition into subtasks, and final review. It reads only what those decisions require and does not carry bulk reading or bulk editing itself
- Substantial code reading (subsystem deep-dives, tracing logic across many files, digesting large diffs): spawn an internal opus subagent - research/Explore/general-purpose with model: opus - and consume only its report. Overrides the sonnet defaults below whenever the task is mostly reading code; sonnet remains for breadth-first locating and non-code research, haiku for purely mechanical scans
- Substantial code modification (multi-file edits, refactors, implementing a decomposed subtask): hand the written spec to an internal opus subagent - code-implementation (opus) - and review the diff it returns. Keep work in the main loop only when it is a small, local edit or needs tight iteration against live session context
- Computer use, GUI automation, and on-device validation: never in the Fable main loop, and not through the in-session codex:codex-rescue wrapper. Route it to a separate root-managed Orca Codex Task/Dispatch that owns the computer-use session, and consume only its text report - see Computer Use / GUI Automation
...
Executor routing:
- **Desktop GUI / device validation, approval-seeded app:** a separate root-managed Orca Codex Task/Dispatch running Codex (GPT-5.5 + `computer-use@openai-bundled`). Nine tools — `click`, `get_app_state`, `type_text`, `list_apps`, `press_key`, `set_value`, `perform_secondary_action`, `scroll`, `drag`. No screenshot tool; `get_app_state` returns screen state. Ask root to open the Task and read back only its report; do NOT drive this through the in-session `codex:codex-rescue` subagent, which cannot clear the approval gate below.
- **Desktop GUI, unseeded app or Codex Task failure:** a haiku subagent (sonnet if the flow needs judgment) loads `mcp__computer-use__*` via ToolSearch and drives the loop. The main loop calls `request_access` first so approval dialogs surface, then hands off. The subagent returns text findings, not screenshots.
- **Web/browser:** same pattern — a haiku/sonnet subagent drives `claude-in-chrome` MCP tools and returns pass/fail plus evidence. The Codex Task route above does not cover the browser: Codex cannot drive a browser from a Claude Code session (verified 2026-07-03: headless `codex exec` runs `danger-full-access` yet `agent.browsers.list()` returns `[]` — `browser@openai-bundled` only controls the in-app browser inside a Codex app session, and `chrome@openai-bundled` requires the Codex Chrome extension, currently not installed). If that extension gets installed, retest `chrome@openai-bundled` as the browser executor.
- **Elicitation gate is per-app.** New apps need a one-time interactive approval that only surfaces when `codex` runs directly in a terminal. This is why GUI work goes to a root-managed Orca Codex Task rather than the in-session `codex:codex-rescue` wrapper: the wrapper sets `CODEX_CI=1` and auto-denies unseen apps, so it can only drive bundle IDs that already have a stored approval. Seed approvals from a real terminal session first, then the Codex Task can drive that app.
